...
$combustibles = isset($_POST['combustibles']) ? json_decode($_POST['combustibles']) : [];

if (!empty($combustibles)) {
    $placeholders = implode(',', array_fill(0, count($combustibles), '?'));
    $sql .= " AND Carburant IN ($placeholders)";
    $params = array_merge($params, (array)$combustibles);
}
